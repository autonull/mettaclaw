...
def _chat(model, content, max_tokens=6000):
    ...
    if model and '/' not in model and not model.startswith('ollama/'):
        # Check if it's likely an openai or anthropic model before blindly prepending ollama
...
def generate_response(model, content, max_tokens=6000):
    """Unified entry point for generating LLM responses using LiteLLM."""
    # This simplifies the old approach which checked the provider name explicitly.
    try:
